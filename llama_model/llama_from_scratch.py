...
#         return ' '.join([self.decode.get(tid, '<unk>') for tid in token_ids])
...
class BPETokenizer:
    ...
    def fit(self, corpus):
        ...
        # Bikin pemetaan balik untuk decoding
    ...
    def decode(self, token_ids):
        """Decode list of token IDs into a readable sentence"""
        tokens = [self.id_to_token.get(tid, "[UNK]") for tid in token_ids]
        # Gabungkan token, hilangkan penanda akhir kata, lalu pisahkan jadi kata
...
if __name__ == "__main__":
    ...
    # Load real dataset
    texts = DatasetLoader.load_text_file('input.txt')
    # Atau gunakan:
    # texts = DatasetLoader.load_json_lines('your_dataset.jsonl')

    # Fit tokenizer
    tokenizer = BPETokenizer()
    corpus = [text.split() for text in texts[:10000]]  # Sampling untuk vocab
    tokenizer.fit(corpus)
    print(f"Vocabulary size: {tokenizer.vocab_size}")

    # Buat dataset
    dataset = TextDataset(texts, tokenizer, max_seq_len=config['max_seq_len'])
    ...
    print("\n=== Starting training ===")
    export_path = trainer.train(dataset, num_epochs=config['num_epochs'], batch_size=config['batch_size'], save_every=1)
    print(f"Model exported to: {export_path}")

    # Load dan test inference
    ...
    loaded_model.eval()
    test_text = "Masukkan kalimat test di sini"
